...
task scoreboard::compare(source_trans s_trans, dest_trans d_trans);
  ...
  if (s_trans.header == d_trans.header) begin
    `uvm_info("SCOREBOARD", "Header PASSED", UVM_LOW)
  end else begin
    `uvm_error("SCOREBOARD", "Header FAILED")
  end

  if (s_trans.payload == d_trans.payload) begin
    `uvm_info("SCOREBOARD", "Payload PASSED", UVM_LOW)
  end else begin
    `uvm_error("SCOREBOARD", "Payload FAILED")
  end

  if (s_trans.parity == d_trans.parity) begin
    `uvm_info("SCOREBOARD", "Parity match successful", UVM_LOW)
  end else begin
    `uvm_error("SCOREBOARD", "Parity failed")
  end
